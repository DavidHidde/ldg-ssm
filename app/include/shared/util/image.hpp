...
namespace shared
{
    ...
    void saveImage(QuadAssignmentTree<DataType> &quad_tree, std::string const filename, size_t const height)
    {
        ...
        for (RowMajorIterator<DataType> it(height, quad_tree); it != it.end(); ++it) {
     ...
     */
    template<typename DataType>
    void saveQuadTreeImages(QuadAssignmentTree<DataType> &quad_tree, std::string const base_filename)
    {
