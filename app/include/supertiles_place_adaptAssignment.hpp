    ...
    namespace place {
        ...
        auto adaptAggregate2Leaf(
            ...
            if (idcs_begin != idcs_end) {
                assert(
                    nodes2leaves[supertileLevelOffset + (*idcs_begin)].size()
                        ==
            ...
            for (
                ...
                if (qtPosIdx != oldQTPosIdx/* || true*/) {
                    ...
                    for (
                        const auto &leafId: helper::range_n(leavesTo.size())) {
...
// #endif
        ...
        void adaptAssignment(
            ...
            auto _adaptAssignmentLeaf = [&supertiles, &tiles, &nodeLeafCounts, &qt, &nElemsTile, &qtLeafAssignment]
                ...
                adaptAssignmentLeaf(
                    idcs_begin, idcs_end, supertiles, tiles, nodeLeafCounts, qt, nElemsTile, qtLeafAssignment,
                    movedFromQTPos
                );
            };
