...
template<
    unsigned int mode, typename F,
    typename A,
    typename PLAN,
    typename DIST_FUNC,/* typename P,*/
    typename IDX,
    typename CACHE
>
#ifdef USE_NVCC
__device__
#endif
thrust::tuple <F, uint32_t, uint32_t> supertiles_assignPartition(
    A &assignments,
    PLAN &plan,
    DIST_FUNC distFunc, /*const P& pos,*/
    const IDX &lidx,
    CACHE v,
    uint32_t planElemOff = 0,
    thrust::tuple
            ...
            if (continueSequencePossible || exchangePossible) {
                ...
                //
                {
                    FindBest_empty_t <A_it, F> rslt;
                    {
                        ...
                        rslt =
                            INTERVOL_NS::findBest_empty<A, F>(
                                abegin,
                                aend,
#ifdef SUPERTILES
...
#pragma omp atomic capture
...
#endif
    {
        ...
        for (
            IDX k = planElemOff; k < nElemsList; k++
            ) {
