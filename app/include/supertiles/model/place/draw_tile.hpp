...
#include "draw_options.hpp"

#include "../../util/place/use_cases.hpp"
...
namespace supertiles
{
    namespace place
    {
        ...
        public:
                ...
                switch (repAggregationType) {
                    case repAggregationType_chart: {
                        ...
                        */
                        ...
                        DIM_TILE dim(8, 8);
                        o = dim * std::min(scaleFac, uint32_t(128));
                    }
                        break;
            ...
            {
                using RO = typename std::remove_const<typename std::remove_reference<decltype(to[0])>::type>::type;
                using ROE = typename std::remove_const<typename std::remove_reference<decltype(to[0].x)>::type>::type;
...
#pragma omp parallel for
                        for (size_t pairIdx = 0; pairIdx < up.size(); pairIdx++) {
                            ...
                            auto from =
                                std::make_pair(get_feat_tileData(lpt.first), get_feat_tileData(lpt.second));
                        ...
                        if(doVR25D)
                          {
                            ...
                            std::random_device r;
                            std::mt19937 rng(r());
                            const auto rslt=strax_mcmc<false>(buf, rep_tileDataDim, opts, rng);
