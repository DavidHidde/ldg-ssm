...
#warning "FOR TESTING FOR NOW, EVEN IN FLOAT3 DO NOT CONSIDER THIRD COMPONENT WHEN HASHING"
    ...
    std::size_t operator()(const float2 &e) const
...
public:
    ...
    supertiles_DistFunc(
        TILE *tiles,
        TILE *supertiles,
        unsigned int *supertilesCnts,
        unsigned int nElems_tile,
        const QT &qt_in,
        const unsigned int *qtNeighbors_in,
    ...
    void init(
        ...
        std::for_each(
            pos_source, pos_source + nElems_source,
            insert_pos2idx
        );
        std::for_each(
            pos_target, pos_target + nElems_target,
            insert_pos2idx
        );
